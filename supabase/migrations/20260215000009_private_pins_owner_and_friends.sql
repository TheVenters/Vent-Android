-- File purpose: Supabase migration that applies the database change described by 20260215000009_private_pins_owner_and_friends.sql.

-- ============================================================================
-- Private pins visibility: owner + accepted friends
-- ============================================================================
...
create policy "Private pins are viewable by owner and friends"
    ...
    and (
      ...
      or exists (
        ...
        where f.status = 'accepted'
          and (
            (f.user_id = public.request_user_id() and f.friend_id = pins.user_id)
            or (f.friend_id = public.request_user_id() and f.user_id = pins.user_id)
          )
      )
    )
  );
