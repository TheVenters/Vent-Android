...
CREATE OR REPLACE FUNCTION "public"."can_view_pin_for_votes"("target_pin_id" "uuid") RETURNS boolean
    ...
    SET "row_security" TO 'off'
    AS $$
  select public.can_actor_view_pin(target_pin_id, public.request_user_id());
$$;
...
CREATE OR REPLACE FUNCTION "public"."can_vote_on_pin"("target_pin_id" "uuid") RETURNS boolean
  ...
  select exists (
    ...
    where p.id = target_pin_id
      ...
      and actor.uid <> p.user_id
      and public.can_actor_view_pin(target_pin_id, actor.uid)
  );
$$;
...
CREATE POLICY "Users can delete their own pins" ON "public"."pins" FOR DELETE USING (("public"."request_user_id"() = "user_id"));
...
CREATE POLICY "Users can insert their own pins" ON "public"."pins" FOR INSERT WITH CHECK (("public"."request_user_id"() = "user_id"));
...
CREATE POLICY "Users can update their own pins" ON "public"."pins" FOR UPDATE USING (("public"."request_user_id"() = "user_id")) WITH CHECK (("public"."request_user_id"() = "user_id"));
...
GRANT ALL ON FUNCTION "public"."join_community"("p_community_id" "uuid") TO "anon";
...
GRANT ALL ON FUNCTION "public"."send_community_message"("p_community_id" "uuid", "p_content" "text") TO "anon";
...
GRANT ALL ON FUNCTION "public"."send_direct_message"("p_receiver_id" "uuid", "p_content" "text") TO "anon";
